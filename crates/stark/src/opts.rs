...
impl SP1ProverOpts {
    ...
    pub fn cpu(cpu_ram_gb: usize) -> Self {
        ...
        opts.recursion_opts.shard_batch_size = std::env::var("RECURSION_SHARD_BATCH_SIZE")
            .ok().and_then(|v| v.parse().ok())
            .unwrap_or(2);
        opts.recursion_opts.records_and_traces_channel_capacity = std::env::var("RECURSION_CHANNEL_CAPACITY")
            .ok().and_then(|v| v.parse().ok())
            .unwrap_or(1);
        opts.recursion_opts.trace_gen_workers = std::env::var("RECURSION_TRACE_GEN_WORKERS")
            .ok().and_then(|v| v.parse().ok())
            .unwrap_or(1);
...
impl SP1CoreOpts {
    ...
    pub fn recursion() -> Self {
        ...
        // Default to 2 concurrent prove workers for recursion.
        // With 12 CPU cores, 2 workers each get ~6 cores for rayon,
        // avoiding the contention from 8 workers fighting for cores.
        opts.shard_batch_size = 2;
        // Allow overriding recursion batch size independently from core.
        if let Ok(val) = env::var("RECURSION_SHARD_BATCH_SIZE") {
            if let Ok(v) = val.parse::<usize>() {
                opts.shard_batch_size = v;
            }
        }
        // Allow overriding recursion trace gen workers independently.
        if let Ok(val) = env::var("RECURSION_TRACE_GEN_WORKERS") {
            if let Ok(v) = val.parse::<usize>() {
                opts.trace_gen_workers = v;
            }
        }
        opts
    }
