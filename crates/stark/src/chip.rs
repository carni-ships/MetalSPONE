...
use super::{
    eval_permutation_constraints_prescoped, generate_permutation_trace_prescoped,
    scoped_interactions, PROOF_MAX_NUM_PVS,
};
...
pub struct Chip<F: Field, A> {
    ...
    /// Pre-scoped local send interactions (avoids per-row clone+sort+hash in eval).
    pub local_sends: Vec<Interaction<F>>,
    /// Pre-scoped local receive interactions (avoids per-row clone+sort+hash in eval).
    pub local_receives: Vec<Interaction<F>>,
    ...
    A: BaseAir<F>,
{
        ...
        A: MachineAir<F> + Air<InteractionBuilder<F>> + Air<SymbolicAirBuilder<F>>,
    {
        ...
        // Precompute scoped interactions to avoid per-row clone+sort+hash in eval.
        let (scoped_sends, scoped_receives) = scoped_interactions(&sends, &receives);
        let local_sends =
            scoped_sends.get(&InteractionScope::Local).cloned().unwrap_or_default();
        let local_receives =
            scoped_receives.get(&InteractionScope::Local).cloned().unwrap_or_default();

        Self { air, sends, receives, local_sends, local_receives, log_quotient_degree }
    }
    ...
    ///
    /// Uses pre-scoped local interactions and cross-row batch inversion.
        ...
        A: MachineAir<F>,
    {
        ...
        generate_permutation_trace_prescoped::<F, EF>(
            &self.local_sends,
            &self.local_receives,
            preprocessed,
            main,
    ...
    pub fn permutation_width(&self) -> usize {
        local_permutation_trace_width(
            self.local_sends.len() + self.local_receives.len(),
    ...
    AB: SP1AirBuilder<F = F> + MultiTableAirBuilder<'a> + PairBuilder + 'a,
{
    fn eval(&self, builder: &mut AB) {
        ...
        // Evaluate permutation constraints using pre-scoped interactions.
        let batch_size = self.logup_batch_size();
        eval_permutation_constraints_prescoped(
            &self.local_sends,
            &self.local_receives,
            batch_size,
            self.air.commit_scope(),
