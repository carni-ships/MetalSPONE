...
#[cfg(target_os = "macos")]
pub type InnerValMmcs = metal_ntt::MetalMmcs;
#[cfg(not(target_os = "macos"))]
...
#[cfg(target_os = "macos")]
pub type InnerDft = metal_ntt::MetalDft;
#[cfg(not(target_os = "macos"))]
...
pub fn sp1_fri_config() -> FriConfig<InnerChallengeMmcs> {
    ...
    let log_blowup = match std::env::var("LOG_BLOWUP") {
        Ok(value) => value.parse().unwrap(),
        Err(_) => 1,
    };
    let pow_bits = match std::env::var("POW_BITS") {
        Ok(value) => value.parse().unwrap(),
        Err(_) => 16,
    };
    FriConfig { log_blowup, num_queries, proof_of_work_bits: pow_bits, mmcs: challenge_mmcs }
}
...
pub mod baby_bear_poseidon2 {
    ...
    #[cfg(target_os = "macos")]
    pub type ValMmcs = metal_ntt::MetalMmcs;
    #[cfg(not(target_os = "macos"))]
    ...
    #[cfg(target_os = "macos")]
    pub type Dft = metal_ntt::MetalDft;
    #[cfg(not(target_os = "macos"))]
