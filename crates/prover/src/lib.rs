...
impl<C: SP1ProverComponents> SP1Prover<C> {
    ...
    pub fn uninitialized() -> Self {
        ...
        let allowed_vk_map: BTreeMap<[BabyBear; DIGEST_SIZE], usize> = if vk_verification {
            ...
            // Dummy VK map for VERIFY_VK=false. Merkle tree height affects compress
            // circuit size (each level = extra hash constraints). Use DUMMY_VK_HEIGHT
            // to control (default 8 for minimal overhead, use 19 to match prod VK map).
            let height: usize = env::var("DUMMY_VK_HEIGHT")
                .ok().and_then(|v| v.parse().ok()).unwrap_or(8);
            let shape_count = 1usize << height;
            tracing::info!("dummy vk map with {} entries (height {})", shape_count, height);
            (0..shape_count)
                .map(|i| ([BabyBear::from_canonical_usize(i); DIGEST_SIZE], i))
                .collect()
        };
    ...
    pub fn compress(
        ...
        // Precompute expected proof counts per height so we can flush partial batches.
        let mut expected_at_height: Vec<usize> = vec![num_first_layer_inputs];
        {
            let mut n = num_first_layer_inputs;
            while n > 1 {
                n = n.div_ceil(batch_size);
                expected_at_height.push(n);
            }
        }
        ...
        let (vk, proof) = thread::scope(|s| {
            ...
            for _ in 0..opts.recursion_opts.shard_batch_size {
                ...
                let handle = s.spawn(move || {
                    let _span = span.enter();
                    // Cache proving keys by program pointer identity. Programs with the
                    // same shape share the same Arc, so pointer equality is correct.
                    // This avoids redundant setup (preprocessed trace gen + PCS commit)
                    // for the ~23 compress proofs that share only a handful of shapes.
                    let mut pk_cache = std::collections::HashMap::<
                        usize,
                        (<C::CompressProver as MachineProver<InnerSC, CompressAir<BabyBear>>>::DeviceProvingKey, StarkVerifyingKey<InnerSC>),
                    >::new();
                    loop {
                            ...
                            received
                        {
                            ...
                            tracing::debug_span!("batch").in_scope(|| {
                                ...
                                // Get the keys, using cache to avoid redundant setup.
                                let cache_key = Arc::as_ptr(&program) as usize;
                                if !pk_cache.contains_key(&cache_key) {
                                    let (pk, vk) = tracing::debug_span!("Setup compress program")
                                        .in_scope(|| {
                                            tracing::debug!("PK cache miss, running setup");
                                            self.compress_prover.setup(&program)
                                        });
                                    pk_cache.insert(cache_key, (pk, vk));
                                } else {
                                    tracing::debug!("PK cache hit, reusing setup");
                                }
                                let t_setup_done = t_proof.elapsed().as_secs_f64();
                                let (pk, vk) = pk_cache.get(&cache_key).unwrap();
                                ...
                                tracing::info!(
                                    "compress proof h={} idx={}: setup={:.0}ms commit={:.0}ms open={:.0}ms total={:.0}ms",
                                    height, index, t_setup_done * 1000.0, commit_ms, open_ms,
                                ...
                                // Send the proof (clone vk from cache).
                                proofs_tx.lock().unwrap().send((index, height, vk.clone(), proof)).unwrap();
            ...
            let handle = {
                ...
                s.spawn(move || {
                    ...
                    // Track how many proofs we've received at each height
                    // so we can flush partial batches when a layer is complete.
                    let mut received_at_height = vec![0usize; expected_at_height.len()];

                    // Helper: send a batch of proofs as a compress input.
                    // Single-element batches use passthrough (is_last=true) to skip
                    // trace gen + proving — the proof is forwarded as-is to the next layer.
                    let send_batch = |batch_inputs: Vec<(usize, usize, StarkVerifyingKey<InnerSC>, ShardProof<InnerSC>)>,
                                          count: &mut usize,
                                          input_sync: &TurnBasedSync,
                                          input_tx: &Arc<Mutex<std::sync::mpsc::SyncSender<_>>>| -> bool {
                        let batch_len = batch_inputs.len();
                        let from_height = batch_inputs[0].1;
                        let next_input_height = from_height + 1;
                        let is_complete = next_input_height == expected_height;
                        let is_passthrough = batch_len == 1;
                        tracing::info!("tree reduce: {} proofs at h={} → compress at h={} (passthrough={}, complete={})",
                            batch_len, from_height, next_input_height, is_passthrough, is_complete);

                        let vks_and_proofs = batch_inputs
                            .into_iter()
                        ...
                        input_tx
                            .lock()
                            .unwrap()
                            .send((*count, next_input_height, input, is_passthrough))
                            .unwrap();
                        input_sync.advance_turn();
                        *count += 1;
                    ...
                    loop {
                        ...
                        if let Ok((index, height, vk, proof)) = received {
                            if height < received_at_height.len() {
                                received_at_height[height] += 1;
                            }
                            batch.push((index, height, vk, proof));

                            // Check if we have a mixed-height batch (layer boundary).
                            let has_mixed_heights = batch.len() > 1
                                && batch.first().unwrap().1 != batch.last().unwrap().1;

                            if has_mixed_heights {
                                // Split: take all elements from the older layer.
                                let old_height = batch[0].1;
                                let split_idx = batch.iter().position(|b| b.1 != old_height).unwrap();
                                let old_layer: Vec<_> = batch.drain(..split_idx).collect();
                                if send_batch(old_layer, &mut count, &input_sync, &input_tx) {
                                    break;
                                }
                                continue;
                            }

                            // All elements at same height. Check if batch is full.
                            if batch.len() >= batch_size {
                                let inputs = std::mem::take(&mut batch);
                                if send_batch(inputs, &mut count, &input_sync, &input_tx) {
                                    break;
                                }
                                continue;
                            }

                            // Batch not full. Check if we've received all proofs for this height
                            // (layer complete, so no more proofs will arrive at this height).
                            let h = batch[0].1;
                            if h < expected_at_height.len()
                                && received_at_height[h] == expected_at_height[h]
                            {
                                let inputs = std::mem::take(&mut batch);
                                if send_batch(inputs, &mut count, &input_sync, &input_tx) {
                                    break;
...
pub mod tests {
    ...
    pub fn run_e2e_prover_with_options<C: SP1ProverComponents>(
        ...
        tracing::info!("setup: {:.3}s", t_setup.elapsed().as_secs_f64());

        let t_core = std::time::Instant::now();
        tracing::info!("prove core");
        let core_proof = prover.prove_core(&pk_d, program, &stdin, opts, context)?;
        tracing::info!("prove_core: {:.3}s", t_core.elapsed().as_secs_f64());
        ...
        compress_span.exit();
        tracing::info!("compress: {:.3}s", t_compress.elapsed().as_secs_f64());
    ...
    pub fn test_e2e_with_deferred_proofs_prover<C: SP1ProverComponents>(
        ...
        let pv_1 = deferred_proof_1.public_values.as_slice().to_vec();
    ...
    /// Benchmark: core + compress only (no gnark/docker).
    /// Set NUM_RUNS env var for multiple iterations (default 5).
    #[test]
    #[serial]
    fn bench_compress() -> Result<()> {
        setup_logger();
        let num_runs: usize =
            env::var("NUM_RUNS").ok().and_then(|v| v.parse().ok()).unwrap_or(5);
        let elf = test_artifacts::FIBONACCI_ELF;
        let opts = SP1ProverOpts::auto();
        let prover = SP1Prover::<CpuProverComponents>::new();
        ...
        for run in 1..=num_runs {
            ...
            bench_e2e_prover::<CpuProverComponents>(
                &prover,
                elf,
                SP1Stdin::default(),
                opts,
            ...
            tracing::info!("run {}/{}: {:.1}s", run, num_runs, elapsed);
            times.push(elapsed);
        }
        times.sort_by(|a, b| a.partial_cmp(b).unwrap());
        let median = times[times.len() / 2];
        tracing::info!("all times: {:?}", times);
