...
pub struct FieldMerkleTree<F, W, M, const DIGEST_ELEMS: usize> {
    pub leaves: Vec<M>,
    /// Preserved dimensions of leaves, valid even after `take_leaves()`.
    pub leaf_dimensions: Vec<Dimensions>,
    ...
    FieldMerkleTree<F, W, M, DIGEST_ELEMS>
{
        ...
        C: Sync,
    {
        ...
        let leaf_dimensions = leaves.iter().map(|m| m.dimensions()).collect();
        Self {
            leaves,
            leaf_dimensions,
            digest_layers,
    ...
    /// Remove and return the leaf matrices, freeing their memory.
    /// Digest layers and leaf_dimensions are preserved for Merkle proofs.
    pub fn take_leaves(&mut self) -> Vec<M> {
        core::mem::take(&mut self.leaves)
    }

    /// Restore leaf matrices previously removed with `take_leaves`.
    pub fn restore_leaves(&mut self, leaves: Vec<M>) {
        self.leaves = leaves;
    }

    /// Whether this tree still has its leaf data.
    pub fn has_leaves(&self) -> bool {
        !self.leaves.is_empty()
    }

    /// Get the maximum height across all leaves (works even after take_leaves).
    pub fn max_height(&self) -> usize {
        self.leaf_dimensions.iter().map(|d| d.height).max().unwrap_or(0)
    }
}
