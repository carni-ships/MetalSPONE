    ...
    [PW::Value; DIGEST_ELEMS]: Serialize + for<'de> Deserialize<'de>,
{
    ...
    fn get_max_height<M: Matrix<P::Scalar>>(&self, prover_data: &Self::ProverData<M>) -> usize {
        // Use leaf_dimensions when leaves have been taken, avoiding panic on empty leaves.
        if prover_data.has_leaves() {
            prover_data.leaves.iter().map(|m| m.height()).max().unwrap()
        } else {
            prover_data.max_height()
        }
    }
...
where
    ...
    [PW::Value; DIGEST_ELEMS]: Serialize + for<'de> Deserialize<'de>,
{
    /// Open a batch using externally-provided leaf matrices and the digest-only tree.
    /// Use when leaves have been taken from the tree to save memory.
    pub fn open_batch_with_external_leaves<M: Matrix<P::Scalar>>(
        &self,
        index: usize,
        prover_data: &FieldMerkleTree<P::Scalar, PW::Value, M, DIGEST_ELEMS>,
        external_leaves: &[M],
    ) -> (Vec<Vec<P::Scalar>>, Vec<[PW::Value; DIGEST_ELEMS]>) {
        let max_height = prover_data.max_height();
        let log_max_height = log2_ceil_usize(max_height);

        let openings = external_leaves
            .iter()
            .map(|matrix| {
                let log2_height = log2_ceil_usize(matrix.height());
                let bits_reduced = log_max_height - log2_height;
